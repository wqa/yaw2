...
| `browse` | *(opt. `path`)* | request a peer's shared listing (§12) |
| `files` | `entries:[{name,size}]` | shared-folder listing |
| `get` | `name` | request a shared file |
| `no-file` | `name` | requested file unknown/refused |
...
Optional, additive, capability-gated. A peer that hosts a folder advertises
`caps:["share"]` in its `hello`. Others may then `browse` and `get` (read-only).

```json
{ "type": "browse" }
{ "type": "files", "entries": [ { "name": "report.pdf", "size": 12345 } ] }
{ "type": "get", "name": "report.pdf" }     →  triggers a normal §11 file-offer
{ "type": "no-file", "name": "report.pdf" } // unknown or refused
```

**Security (mandatory):** the host MUST treat `get.name` as untrusted — serve a file
only if `name` is a single safe path component **inside** the configured share dir
(no `/`, `\`, `..`, no leading `.`, resolved real path's parent == share dir, regular
file). Otherwise reply `no-file`. See [`extensions/file-browse.md`](extensions/file-browse.md).
